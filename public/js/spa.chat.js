...
spa.chat = (function() {
	...
	var configMap = {
		...
		slider_opened_em: 22, //18
		slider_opened_min_em: 10,
		window_height_min_em: 24,//20
		slider_closed_em: 2,
	...
	setSliderPosition = function(position_type, callback) {
		...
		//therefore simply return false; the shell will fix the
		//uri and try again.
	...
	onSetchatee = function(event, arg_map) {
		...
		if (!new_chatee) {
			if (old_chatee) {
				writeAlert(old_chatee.name + ' has left the chat room.');
			}else {
				writeAlert('Your friend has left the chat rooom.');
			}
